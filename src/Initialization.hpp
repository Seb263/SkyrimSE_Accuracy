...
namespace ModData
{
	...
	public:
		...
		void PostLoadData()
		{
			...
			ModCore::Impacts::RegisterImpacts();
		}
	...
	private:
		...
		static inline void ExtractGameAssets()
		{
			constexpr unsigned char PscBytes[] = {
				#include "Accuracy.psc.h"
			};

			constexpr unsigned char PexBytes[] = {
				#include "Accuracy.pex.h"
			};
			...
			const std::array<AssetEntry, 2> assets{{
				{ PscData, "Data/Source/Scripts/Accuracy.psc", true },
				{ PexData, "Data/Scripts/Accuracy.pex", false }
			}};
		...
		static inline bool LoadCIFApi()
		{
			...
			constexpr REL::Version kRequiredVersion{ 2, 0, 3, 0 };
			...
			logger::info("Core Impact Framework API registered successfully.");
